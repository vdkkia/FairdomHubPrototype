module SamplesHelper
  def sample_form_field_for_attribute(attribute)
    ...
    when Seek::Samples::BaseType::CV
      ...
      is_free_text = !attribute.sample_controlled_vocab.source_ontology.blank?
      existing_objects = []
      existing_objects << Struct.new(:id,:name).new(value,value) if value
      objects_input(element_name, existing_objects, typeahead:  {query_url: typeahead_sample_controlled_vocabs_path + "?query=%QUERY&scv_id=#{scv_id}",handlebars_template:'typeahead/controlled_vocab_term'}, limit:1, ontology: is_free_text)
