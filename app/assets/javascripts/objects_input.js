function loadObjectInputs() {
  $j('[data-role="seek-objectsinput"]').each(function () {
    var options = { tagClass: "label label-default", itemValue: "id", itemText: "name" };

    if ($j(this).data("tagsLimit")) options.maxTags = $j(this).data("tagsLimit");
    ...
    if ($j(this).data("typeahead")) {
      ...
      if ((prefetchUrl = $j(this).data("typeahead-prefetch-url"))) opts.prefetch = { url: prefetchUrl };
      if ((queryUrl = $j(this).data("typeahead-query-url"))) opts.remote = { url: queryUrl };
      if ((localValues = $j(this).data("typeahead-local-values"))) opts.local = localValues;
