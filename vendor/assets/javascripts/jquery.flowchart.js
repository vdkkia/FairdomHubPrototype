  ...
  $j.widget("flowchart.flowchart", {
    ...
    _initEvents: function () {
      ...
      this.objs.layers.operators.on("click", ".flowchart-operator", function (
        ...
        if ($j(e.target).closest(".flowchart-operator-connector").length == 0) {
          self.selectOperator($j(this).data("operator_id"));
        }
      });
        ...
        function () {
          ...
          if (self.options.canUserEditLinks) {
            self._connectorClicked(
              $jthis.closest(".flowchart-operator").data("operator_id"),
              $jthis.data("connector"),
              $jthis.data("sub_connector"),
              $jthis
      ...
      this.objs.layers.links.on("mouseover", ".flowchart-link", function () {
        self._connecterMouseOver($j(this).data("link_id"));
      });

      this.objs.layers.links.on("mouseout", ".flowchart-link", function () {
        self._connecterMouseOut($j(this).data("link_id"));
      });

      this.objs.layers.links.on("click", ".flowchart-link", function () {
        self.selectLink($j(this).data("link_id"));
      });
      ...
      this.objs.layers.operators.on(
        ...
        ".flowchart-operator",
        function (e) {
          self._operatorMouseOver($j(this).data("operator_id"));
        }
      );
      ...
      this.objs.layers.operators.on(
        ...
        ".flowchart-operator",
        function (e) {
          self._operatorMouseOut($j(this).data("operator_id"));
        }
      );
    },
    ...
    createLink: function (linkId, linkDataOriginal) {
      var linkData = $j.extend(true, {}, linkDataOriginal);
    ...
    getOperatorCompleteData: function (operatorData) {
      ...
      var infos = $j.extend(true, {}, operatorData.internal.properties);
    ...
    _getOperatorFullElement: function (operatorData) {
      ...
      if (operatorData.properties.shape == "oval") {
        ...
        div = '<div class="flowchart-operator parallelogram"></div>';
        op_title =
          '<div class="flowchart-operator-title anti_parallelogram"></div>';
      }
      var $joperator = $j(div);
      $joperator.addClass(infos.class);
      //anti_parallelogram
      var $joperator_title = $j(op_title);
      $joperator_title.html(infos.title);
      // $joperator_title.html("OPERATOR");

      $joperator_title.appendTo($joperator);

      var $joperator_inputs_outputs = $j(
        '<div class="flowchart-operator-inputs-outputs"></div>'
      );

      $joperator_inputs_outputs.appendTo($joperator);

      var $joperator_inputs = $j(
        '<div class="flowchart-operator-inputs"></div>'
      );
      $joperator_inputs.appendTo($joperator_inputs_outputs);

      var $joperator_outputs = $j(
        '<div class="flowchart-operator-outputs"></div>'
      );
      $joperator_outputs.appendTo($joperator_inputs_outputs);
      ...
      function addConnector(
        ...
        var $joperator_connector_set = $j(
          '<div class="flowchart-operator-connector-set"></div>'
        );
        $joperator_connector_set.data("connector_type", connectorType);
        $joperator_connector_set.appendTo($joperator_container);

        connectorArrows[connectorKey] = [];
        connectorSmallArrows[connectorKey] = [];
        connectors[connectorKey] = [];
        connectorSets[connectorKey] = $joperator_connector_set;
      ...
      for (var key_i in infos.inputs) {
        if (infos.inputs.hasOwnProperty(key_i)) {
          addConnector(key_i, infos.inputs[key_i], $joperator_inputs, "inputs");
        }
      }
    ...
    _createSubConnector: function (connectorKey, connectorInfos, fullElement) {
      var $joperator_connector_set = fullElement.connectorSets[connectorKey];

      var subConnector = fullElement.connectors[connectorKey].length;

      var $joperator_connector = $j(
        '<div class="flowchart-operator-connector"></div>'
      );
      $joperator_connector.appendTo($joperator_connector_set);
      $joperator_connector.data("connector", connectorKey);
      $joperator_connector.data("sub_connector", subConnector);

      var $joperator_connector_label = $j(
        '<div class="flowchart-operator-connector-label"></div>'
      );
      $joperator_connector_label.text(
        connectorInfos.label.replace("(:i)", subConnector + 1)
      );
      $joperator_connector_label.appendTo($joperator_connector);

      var $joperator_connector_arrow = $j(
        '<div class="flowchart-operator-connector-arrow"></div>'
      );

      $joperator_connector_arrow.appendTo($joperator_connector);

      var $joperator_connector_small_arrow = $j(
        '<div class="flowchart-operator-connector-small-arrow"></div>'
      );
      $joperator_connector_small_arrow.appendTo($joperator_connector);

      fullElement.connectors[connectorKey].push($joperator_connector);
      fullElement.connectorArrows[connectorKey].push(
        $joperator_connector_arrow
      );
      fullElement.connectorSmallArrows[connectorKey].push(
        $joperator_connector_small_arrow
      );
    },
        ...
        fullElement.operator.draggable({
          ...
          start: function (e, ui) {
            ...
            pointerX =
              ...
              parseInt($j(e.target).css("left"));
            pointerY =
              (e.pageY - elementOffset.top) / self.positionRatio -
              parseInt($j(e.target).css("top"));
          },
          drag: function (e, ui) {
            if (self.options.grid) {
              ...
              if (!operatorData.internal.properties.uncontained) {
                ...
                ui.position.left = Math.min(
                  ...
                  self.element.width() - $jthis.outerWidth()
                );
                ui.position.top = Math.min(
                  Math.max(ui.position.top, 0),
                  self.element.height() - $jthis.outerHeight()
                );
              }
            ...
            operatorChangedPosition($j(this).data("operator_id"), ui.position);
          },
          stop: function (e, ui) {
            self._unsetTemporaryLink();
            var operatorId = $j(this).data("operator_id");
    ...
    _click: function (x, y, e) {
      ...
      if ($jtarget.closest(".flowchart-operator-connector").length == 0) {
        this._unsetTemporaryLink();
      }

      if ($jtarget.closest(".flowchart-operator").length == 0) {
        this.unselectOperator();
      }

      if ($jtarget.closest(".flowchart-link").length == 0) {
        this.unselectLink();
      }
    },
    ...
    getData: function () {
      ...
      var data = {};
      data.operators = $j.extend(true, {}, this.data.operators);
      data.links = $j.extend(true, {}, this.data.links);
    ...
    getOperatorData: function (operatorId) {
      var data = $j.extend(true, {}, this.data.operators[operatorId]);
    ...
    getOperatorFullProperties: function (operatorData) {
      if (typeof operatorData.type != "undefined") {
        ...
        return $j.extend({}, typeProperties, operatorProperties);
      } else {
